...
outputEncoding: utf-8 | latin-1 jne
...
FetchStream.prototype.runStream = function(url){
    ...
    var httpByIP = (function (ip) {
        console.log('httpByIP:' + ip)
        ...
        url_data.urloptions.host=ip+portAddition;
 
        this._runStream(url_data, url);
    }).bind(this);

    //console.log(this._redirect_count)
    if (this.options.ip) {
        this.first_host = this.first_host || url_data.urloptions.host;
        if (this._redirect_count==0 || url_data.urloptions.host==this.first_host) {
            httpByIP(this.options.ip);
            return;
        } else {
            delete url_data.urloptions.headers['host'];
        }
    }

    if (this.options.asyncDnsLookup && !net.isIP(url_data.urloptions.host)) {
...
FetchStream.prototype._runStream = function(url_data, url){
    ...
    var req = url_data.transport.request(url_data.urloptions, (function(res) {
        ...
        this.ip = this.ip || res.connection.remoteAddress;
    ...
    if (this.options.timeout) {
        this.timeout = setTimeout(function(){
            this.emit('error', new Error('Http timeout'));
